...
>(
  ...
  async ({ orderId, newStatus }, { dispatch, getState, rejectWithValue }) => {
    try {
      ...
      // Create updated order with timestamp
...
/**
 * Format order ID for display (remove timestamp/random parts if present)
 */
const formatOrderId = (orderId: string): string => {
  // If using format like "ORD-1234567890-ABC12", show as "ORD-...ABC12"
  if (orderId.includes('-')) {
